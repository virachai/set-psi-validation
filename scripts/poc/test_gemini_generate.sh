...
if [ -z "$API_KEY" ]; then
    ...
    echo "Usage:"
    echo "  1) Via Argument:  ./test_gemini_generate.sh <YOUR_API_KEY> [MODEL] [INPUT_PROMPT]"
    echo "  2) Via Env Var:   export GEMINI_API_KEY='your_key' && ./test_gemini_generate.sh"
    exit 1
fi
...
REQUEST_FILE="${SCRIPT_DIR}/request_generate.json"
...
echo "Calling Gemini generateContent API (single-shot, non-streaming)..."
echo "=========================================="
